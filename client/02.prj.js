...
imgs.map(({url}, index) => {
  ...
  img_box.index = index // 이렇게 해도 괜찮은가? 오염시키는 느낌?
...
function check_mark_arrow() {
  if(light_box.index === 0) {light_box__img.removeChild(light_box__left_btn)}
  if(light_box.index === 1) {light_box__img.appendChild(light_box__left_btn)}
  if(light_box.index === imgs.length-1) {light_box__img.removeChild(light_box__right_btn)}
  if(light_box.index === imgs.length-2) {light_box__img.appendChild(light_box__right_btn)}
}
